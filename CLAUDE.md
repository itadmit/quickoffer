...
`app_settings` ב-Neon: **`llm.provider=openai`, `llm.model=gpt-4o-mini`; `transcription` נשאר `groq/whisper-large-v3`.** אומת קצה-לקצה דרך ההגדרות עצמן. הנימוק: OpenAI Tier 1 נותן 200,000 TPM מול 8,000, האיכות זהה (20/21), והעלות נמוכה יותר כי אין טוקני reasoning. התמלול נשאר ב-Groq כי הוא חינם ועברית שלו מאומתת.

**החסם עבר:** כבר לא טוקנים לדקה אלא **2,000 תמלולים ליום** ב-Groq החינמי. הכרטיס באדמין מזהה את זה נכון (`bottleneck: transcription-requests`).
