...
#  24.6  his smile              26.1  end card + the free offer
#
...
ffmpeg -nostdin -loglevel error -y \
  ...
  -i "$A/send.mp3" -i "$A/ui.mp3" -i "$A/ding.mp3" -i "$A/pad1.mp3" \
  -filter_complex "
  ...
  [4:a]asplit=4[u1][u2][u3][u4];
  [3:a]asplit=2[w1][w2];
  [u1]volume=${UI_G},adelay=6580|6580[s1];
  [u2]volume=${UI_G},adelay=8000|8000[s2];
  [w1]volume=${SEND_G},adelay=17320|17320[s3];
  [u3]volume=${UI_G},adelay=17950|17950[s4];
  [5:a]volume=${DING_G},adelay=19700|19700[s5];
  [u4]volume=${UI_G},adelay=22750|22750[s6];
  [w2]volume=${SEND_G},adelay=23860|23860[s7];
  [s1][s2][s3][s4][s5][s6][s7]amix=inputs=7:normalize=0[SFX];

  [6:a]atrim=0:28.8,asetpts=PTS-STARTPTS,afade=t=in:st=0:d=2.0,afade=t=out:st=26.8:d=2.0,
  ...
  [VOX][VN][AMB][SFX][MUSD]amix=inputs=5:normalize=0:duration=longest,
    aresample=48000,aformat=channel_layouts=stereo,atrim=0:33.4[MIX]
