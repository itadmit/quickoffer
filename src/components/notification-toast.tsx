 ...
 * Each item pops in, holds, fades out, then the next one pops in.
 */
const ITEMS: { icon: LucideIcon; text: string }[] = [
  { icon: Send, text: "הצעת מחיר #1042 נשלחה לדני כהן (767 ₪)" },
  { icon: Eye, text: "דני כהן פתח את הצעה #1042" },
  { icon: BadgeCheck, text: "דני כהן אישר וחתם על הצעה #1042 (767 ₪)" },
...
export function NotificationToast({ className = "" }: { className?: string }) {
  ...
  const { icon: Icon, text } = ITEMS[index];
